...
public class MarkovChain {
	...
	public void addLine(String line) {
		lock.writeLock().lock();
		{
			...
			for (int i = 0; i < currentWords.size() - 1; i++) {
				String currentWord = Util.selectivelyLowerCase(currentWords
						.get(i));
				String nextWord = Util.selectivelyLowerCase(currentWords
						.get(i + 1));
	...
	public String reply(String inputString, String name, String sender) {
		...
		for (int i = 0; i < currentWords.size(); i++) {
			...
			if (bestList == null
					|| bestList.size() == 0
					|| (list != null && list.size() > 0 && (list.size() < bestList
							.size()))) {
				bestWordPairString = pairKey;
				bestWordPair = i;
				// System.out.println("Best word pair " + bestList + " " + list
				// + " " + bestWordPairString + " " + bestWordPair);
			}
			...
			if (bestList == null
					...
					|| (list != null && list.size() > 0 && list.size() < bestList
							.size())) {
				bestWord = i;
				// System.out.println("Best word " + bestList + " " + list + " "
				// + bestWord);
			}
		...
		if ((bestList = wordPairsNext.get(bestWordPairString)) != null
				&& bestList.size() > 0 && rand.nextDouble() > .05) {
			// String word =
			// words.get(list.get(rand.nextInt(list.size()))).toString();
			// System.out.println("Adding \"" + bestWordPairString +
		...
		for (int size = sentence.size() - 1; size < sentence.size();) {
			...
			if ((wordIndex = currentWords.indexOf(previousWord)) != -1
					&& wordIndex < currentWords.size() - 1
					&& wordObj != null
					&& wordObj.getCount() / (double) lines.size() < rand
							.nextDouble()) {
			...
			if (list != null
					&& rand.nextDouble() < inputLength
							/ (double) sentence.size()) {
				String word = words.get(list.get(rand.nextInt(list.size())))
						.toString();
				// System.out.println("Adding " + word +
				// " to sentence from pair " + key);
				sentence.add(word);
				...
				if (list != null) {
					String word = words
							.get(list.get(rand.nextInt(list.size())))
							.toString();
					// System.out.println("Adding " + word +
					// " to sentence from word " + key);
					sentence.add(word);
					...
					if (wordsNext.get(word) != null)
						wordFrequency = words.get(words.lookup(word))
								.getCount();
					// System.out.println(word + ": " + wordFrequency /
					// (double)sentence.size());
					if (rand.nextDouble() > (wordFrequency / (double) sentence
							.size())) {
						break;
					}
				}
	...
	public String command(String command) {
		...
		case "STATS":
			return "Word pair contexts: " + wordPairsNext.size()
					+ ", word contexts: " + wordsNext.size() + ", words: "
			...
			if (parts.size() > 1) {
				...
				if (words.lookup(wordString) != null) {
					...
					if (wordsNext.get(wordString.toLowerCase()) != null)
						for (Integer index : wordsNext.get(wordString
								.toLowerCase())) {
							...
							for (int i = 0; i < nextWords.size(); i++)
								if (nextWords.get(i)[0] == index) {
									...
									if (nextWords.get(i)[1] > nextWordCount) {
										...
										nextWordString = words.get(
												nextWords.get(i)[0]).toString();
									}
									notFound = false;
									break;
								}
